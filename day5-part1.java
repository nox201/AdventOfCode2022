...
public class main {
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i = 0; i < instructions.size(); i++) {
			...
			//ITERATE NUMBER OF TIMES TO MOVE CRATES
			for(int j = 0; j < Integer.valueOf(parts[1]); j++) {
				moveCrate(Integer.valueOf(parts[3]) - 1, Integer.valueOf(parts[5]) - 1);
				debugCrates();
			}
	...
	public static void moveCrate(int fromStack, int toStack) 
	{
		//ADD VALUE FROM FROMSTACK TO TOSTACK
		crates.get(toStack).add(crates.get(fromStack).get(crates.get(fromStack).size() - 1));
